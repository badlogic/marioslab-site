...
[Last time](../babys-first-pixel/), we set up our development environment and explored concepts like pixels and rasters. Today, we're going to build on that knowledge and figure out how to draw rectangles. Exciting!

But before we do that, let's abstract pixel rasters, setting and getting pixels, and a few other small things so we'll never have to think about them hard ever again.
...
Let's stuff a few things into `src/r96/r96.h` and `src/r96/r96.c`, where re-usable code used by the demo apps lives. I'll just highlight the additions through excerpting and link to lines in the corresponding file on GitHub.
...
Direct call to `malloc()`, `sizeof()`, casting, yuck! How about we wrap all this in a handful of simple macros. I did just that in [`src/r96/r96.h`](https://github.com/badlogic/r96/blob/dont-be-square-00/src/r96/r96.h#L12-L14):
...
`R96_ALLOC(type)` allocates one instance of `type` for you. If you want an array of something, use `R96_ALLOC_ARRAY(type, numElements)`. We also want to clean up after ourselves, so we also have `R96_FREE(ptr)`. E.g.:
...
Getting the individual color components out of a color constructed with `R96_ARGB()` is a bit tedious. Let's fix that with more macros in [src/r96/r96.h](https://github.com/badlogic/r96/blob/dont-be-square-00/src/r96/r96.h#L7-L10):
...
Let's create a struct in [`src/r96/r96.h`](https://github.com/badlogic/r96/blob/dont-be-square-00/src/r96/r96.h#L16-L19) to ease the pain:
...
Why use signed integers for width and height? Many of the algorithms we'll be building will work using signed integers or even floats. Casting from `uint32_t` or `size_t` all the time isn't fun. We surely won't ever put a negative value in those fields, right?
...
While we won't go for a full ["handles instead of pointers"](https://floooh.github.io/2018/06/17/handles-vs-pointers.html) system for the `r96` library, we'll strive to make our code as pointer- and allocation-free as possible. This will enable building apps on top of the `r96` library code, that themselves can then implement a handle based system.
...
- Resource-owning types are initialized and disposed with corresponding initializer and disposal functions. E.g. `r96_image`, `r96_image_init()`, and `r96_image_dispose()`
- Resource-owning types are always passed and returned by reference
- Non-resource-owning types are initialized with [C99 designated initializers](https://gcc.gnu.org/onlinedocs/gcc-4.1.2/gcc/Designated-Inits.html)
- Non-resource-owning types are passed to and returned from functions by value, unless performance considerations make it prohibitive
...
Manually calculating the address of a pixel is annoying, so let's add functions that can read and write pixels from and to a `r96_image`:
...
In the last demo app we made sure to draw pixels within the bounds of the image. For the two more general purpose functions `r96_set_pixel()` and `r96_get_pixel()` we can not assume that the input is valid. We therefore make sure that the provided `x` and `y` coordinates are within the bounds of the image, before actually accessing the pixel. This is known as [clipping](https://en.wikipedia.org/wiki/Clipping_(computer_graphics)) Clipping will be a permanent, very annoying companion of ours throughout this journey.

> **Note:** the largest x-coordinate that's still within the bounds of the image is `width - 1`, the largest y-coordinate is `height - 1`. That `-1` is a constant source of errors. I still mess this up from time to time, even after 25 years of graphics programming. [No, really](https://github.com/badlogic/r96/commit/6f00a0ce610c09d1e39b058764d9b10cee27c0f3). It's not even funny.
...
Let's put our new fancy API to use and create a new demo app called [`02_raster.c`]() (see [the last article](/posts/rendering-like-its-1996/babys-first-pixel/#Demo%20app:%20drawing%20a%20pixel) on how to add a new demo app to the project). It does pretty much the same thing as the last demo app, except it uses the new API.
...
In lines 6-8, we create a new `r96_imafe` instance and initialize it to have 320x240 pixels. The pixel rendering in line 14 now uses `r96_set_pixel()` instead of manually calculating the pixel address. To exercise the new `r96_get_pixel()` function and the color component macros, we print the current color at the mouse position if the left mouse button is pressed. Finally, we dispose of the image in line 26. Click the demo below to start it.
...
Often, we want to set all pixels in an image to black before we start drawing. `r96_clear()` covers exactly that common case. It uses `memset()`, which is heavily optimized and usually beats a hand-rolled loop. Sadly, we can't use `memset()` to set 32-bit values like we'd need if we wanted to specify a `uint32_t` encoded color to clear the image with.

`r96_clear_with_color()` lets us clear with an arbitrary color. As we loop through pixels individually, it will likely be quite a bit slower than the `memset()` based alternative above. But is it really?
...
Luckily, profiling `r96_clear()` and `r96_clear_with_color()` is comparatively trivial. All we need is a [high-resolution timer](http://www.songho.ca/misc/timer/timer.html) that can give us nano-second precision. MiniFB comes with such a [high precision timer](https://github.com/emoon/minifb#timers-and-target-fps). Let's write a command line app called [`03_clear_profiling.c`]() that tests how fast `r96_clear()` and `r96_clear_with_color()` are:
...
We create a 320x200 pixels image and a timer. As a base-line, we measure how long it takes to clear the image `200000` times using `r96_clear()`. Then we measure how long it takes to do the same with `r96_clear_with_color()`. We always profile our code using the release build. Here's what I get on my machine:
...
Alright, that's absolutely not great. We assumed that manually looping through and setting the pixels is a bit slower than `memset()`. But 8.5x times slower?! There must be something wrong.
...
> **Note:** while you can get the assembly output straight from your compiler, I tend to use the [Godbolt Compiler Explorer](https://godbolt.org), an online C/C++ compiler and disassembler. Enter C/C++ code in the left side editor and have Godbolt show you the assembly output by the choosen compiler + flags on the right side. You can even [share your sketches](https://www.godbolt.org/z/Przd11aPe), so others can help improve things easily, without needing to set up a build environment.
...
Lines 2-5 multiply `image->height` (`[rdi + 4]`) by `image->width` (`[rdi]`), then check if the result is zero. If that's the case, we jump to `.LBB1_3` and exit the function. This part checks if there's any need to iterate at all and bails out early.
...
In line 11 we load `image->width` into `rdx`, followed by loading `image->height` into `rax` in line 12, which are then multiplied in line `13`, with the result ending up in `rax`. `rax` is then compared to `i` (`rcx`). If it is less than `image->width * image->height` (`rax`), we jump back to `.LBB1_2` and write the next pixel, otherwise we fall through and exit the function.
...
We define a horizontal line by a start point `(x1, y)` and end `(x2, y)`, where `x1 <= x2`. Since it's a horizontal line, the y-coordinate is the same for both points. The pixels the start and end point are located in, as well as the pixels in between the two points are all part of the horizontal line. A few example horizontal lines on a small 16x12 raster.
...
<p>
	The <span style="color: #e0e">ping</span> lines are ... weird. They are partially in the raster. How do we deal with them? We clip them! For the left point, we snap `x1` to `0`. For the right point, we snap `x2` to `width - 1`. Here are all the pixels we actually have to draw.
</p>
...
Once we've clipped a line, we just need to draw all the pixels between its start and end point.

### Demo app: horizontal lines, naive version

Let's add a new demo app called [`04_hline.c`]():
...
The `hline()` function first ensures that `x1 < x2` in lines 6-10. If we didn't guarantee this, the following code would be quite a bit more complicated.

Next, we reject all lines that are entirely outside the image, applying the rules we established above (lines 12-15)

Lines that are partially inside the image get clipped in lines 17 and 18.

With all the clipping out of the way, we know that the start and end point (and the pixels between them) are entirely within the bounds of the image. We finish the function by drawing all the pixels at and in between the start and end point in lines 20-21 using `r96_set_pixel()`

In the `main()` function, we draw `200000` lines at random positions and measure how long it takes to do so. The call to `srand()` sets the seed for the function `rand()`. We'll always get the same sequence of "random" numbers that way. If we measure something to later improve it, we have to ensure that what you measured is the same each time, even if it involves "fake" randomness. Here's the output of a release build on my machine:
...
Oh right, it actually does clipping and it calculates the address of the pixel, which adds an addition and multiplication. That's not good! The `hline()` function already ensures that all pixels we are drawing are inside the raster. The extra clipping in `r96_set_pixel()` is thus unnecessary. And once we know the address of the start pixel at `x1, y`, the address of each subsequent pixel on the line can be can be calculated by an increment of 1. Remember: pre-calculate all loop invariants! Here's the optimized `hline()` version from [`05_hline_opt.c`]():
...
We keep all the clipping from before, but are a bit smarter regarding our inner loop. We precalculate the address of the pixel at coordinates `(x1, y)` in line 16. We also precalculate the number of pixels we are going draw. The inner loop then counts down the number of pixels, while assigning the color to the current pixel and incremening the address by one. Pretty tight. On my machine, I get these timings:
